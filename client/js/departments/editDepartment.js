...
const editDepartment = (uid) => {
  const [did] = getParamsFromUrl(["did"]);
  if (!did) useRedirect("departments.html", {
    uid
  })
  const dep = {
  ...
  crud.options = Object.assign(crud.options, generateBody(dep, "PUT"));
  crud.put().then(() => {
    setTimeout(() => {
...
const initializeEmployeeList = async () => {
  ...
  if (auth) {
    ...
    departElements.saveDepartment.onclick = () => editDepartment(uid)
  } else {
    useKickout();
  }
}
